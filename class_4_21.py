...
def car(env): #def 함수선언. def 함수이름(인수)
    while True: # While True 반복문
        ...
        yield env.timeout(parking_duration) # yield event(time) 형태이지만, event 대신 process(function)가 올 경우, 그 process가 끝날 때까지 진행한다.
...
env = simpy.Environment() # Environment를 만드는 것.
env.process(car(env)) # simulation 대상을 env에 등록시킴.
